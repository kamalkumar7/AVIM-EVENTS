...
export default function EnquiriesPage() {
  ...
  return (
    <div className="p-8 max-w-5xl">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-white">Enquiries</h1>
        <p className="text-gray-400 text-sm mt-0.5">Contact form submissions. {unread > 0 && <span className="text-amber-400">{unread} unread.</span>}</p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-700">
              ...
              <th className="text-left px-5 py-3 text-xs text-gray-400 uppercase tracking-wider">Event</th>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className={`border-b border-gray-800 hover:bg-gray-800/30 ${!item.read ? "bg-amber-500/5" : ""}`}>
                ...
                <td className="px-5 py-3 text-gray-400">{item.eventType || "—"}</td>
