    ...
    <div className="p-8 w-full">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Venue partner cards on the /venues page.</p>
        </div>
          ...
          <div key={group.value} className="mb-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {group.items.map((item) => (
                <div key={item.id} className={`bg-white border border-gray-200 rounded-xl overflow-hidden group ${!item.active ? "opacity-50" : ""}`}>
                  <AdminImage src={item.imageUrl} alt={item.name} className="w-full h-28 object-cover" />
                  <div className="p-3">
                    <div className="flex items-center gap-1 flex-wrap">
                      <p className="text-gray-900 text-xs font-semibold">{item.name}</p>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm" placeholder="e.g. Taj Palace" />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Location</label>
              <input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm" placeholder="e.g. New Delhi, India" />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Tag</label>
              <input value={form.tag} onChange={(e) => setForm({ ...form, tag: e.target.value })} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm" placeholder="e.g. Featured" />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Tier *</label>
              <select value={form.tier} onChange={(e) => setForm({ ...form, tier: e.target.value })} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm">
