  ...
  return (
    <div className="p-8 w-full">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={openAdd} className="bg-gray-50 hover:bg-white text-gray-900 text-sm font-semibold px-4 py-2 rounded-lg transition-colors">+ Add Item</button>
      </div>
        ...
        {items.map((item) => (
          <div key={item.id} className={`relative rounded-xl overflow-hidden border border-gray-200 group shadow-sm ${!item.active ? "opacity-50" : ""}`}>
            ...
            <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onClick={() => toggle(item)} className={`text-[10px] px-1.5 py-0.5 rounded ${item.active ? "bg-emerald-500/80 text-gray-900" : "bg-gray-500/80 text-gray-900"}`}>{item.active ? "On" : "Off"}</button>
              <button onClick={() => openEdit(item)} className="text-[10px] bg-white/90 text-gray-700 px-1.5 py-0.5 rounded shadow-sm">Edit</button>
              <button onClick={() => remove(item.id)} className="text-[10px] bg-red-500/80 text-gray-900 px-1.5 py-0.5 rounded">Del</button>
            </div>
          </div>
        ))}
        {initialLoad ? (
