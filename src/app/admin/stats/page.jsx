  ...
  return (
    <div className="p-8 w-full">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Stats & Counters</h1>
          <p className="text-gray-500 text-sm mt-0.5">Animated number counters on Home and About pages.</p>
        </div>
          ...
          <div key={group.value} className="mb-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {group.items.map((item) => (
                <div key={item.id} className={`bg-white border border-gray-200 rounded-xl p-4 ${!item.active ? "opacity-50" : ""}`}>
                  <p className="text-2xl font-bold text-amber-400">{item.value}{item.suffix}</p>
                  <p className="text-gray-900 text-xs font-medium mt-1">{item.label}</p>
                  {item.sublabel && <p className="text-gray-500 text-[10px] mt-0.5">{item.sublabel}</p>}
                  <div className="flex gap-2 mt-3">
                    <button onClick={() => openEdit(item)} className="text-[10px] text-gray-500 hover:text-gray-900">Edit</button>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? "Edit Stat" : "Add Stat"}>
        <div className="space-y-4">
          <div>
            ...
            <select value={form.section} onChange={(e) => setForm({ ...form, section: e.target.value })} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" value={form.value} onChange={(e) => setForm({ ...form, value: +e.target.value })} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm" />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Suffix</label>
              <input value={form.suffix} onChange={(e) => setForm({ ...form, suffix: e.target.value })} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm" placeholder="+" />
            </div>
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1">Label *</label>
            <input value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm" placeholder="e.g. Events Executed" />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1">Sub-label</label>
            <input value={form.sublabel} onChange={(e) => setForm({ ...form, sublabel: e.target.value })} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm" placeholder="e.g. pan India & international" />
          </div>
