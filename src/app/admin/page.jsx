        ...
        {tiles.map((t) => {
          ...
          return (
            <Link
              key={t.href + t.label}
              href={t.href}
              className={`bg-white border rounded-xl p-5 hover:-translate-y-1 hover:shadow-md transition-all duration-300 group relative ${
                t.urgent ? "border-amber-300 shadow-sm" : "border-gray-200 hover:border-gray-300"
              }`}
            >
              {t.urgent && (
                <span className="absolute top-3 right-3 w-2 h-2 rounded-full bg-amber-500 animate-pulse shadow-[0_0_8px_rgba(217,156,15,0.4)]" />
              )}
