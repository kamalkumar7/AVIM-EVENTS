    ...
    <div className="p-8 w-full">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Hotel & venue property logos shown on the homepage.</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mb-6">
        {items.map((item) => (
          <div key={item.id} className={`bg-white border border-gray-200 rounded-xl p-4 flex flex-col items-center gap-3 group relative ${!item.active ? "opacity-50" : ""}`}>
            ...
            <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onClick={() => openEdit(item)} className="text-[10px] bg-gray-700 hover:bg-gray-600 text-gray-900 px-1.5 py-0.5 rounded">Edit</button>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? "Edit Property" : "Add Property"}>
        <div className="space-y-4">
          <div>
            ...
            <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm" placeholder="e.g. JW Marriott" />
          </div>
