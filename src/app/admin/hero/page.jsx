  ...
  return (
    <div className="p-8 w-full">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={openAdd} className="bg-gray-50 hover:bg-white text-gray-900 text-sm font-semibold px-4 py-2 rounded-lg transition-colors">+ Add Slide</button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {slides.map((slide, i) => (
          <div key={slide.id} className={`relative rounded-xl overflow-hidden border ${slide.active ? "border-gray-200" : "border-gray-200 opacity-50"} group shadow-sm`}>
            ...
            <button onClick={() => toggle(slide)} className={`absolute top-2 right-2 text-[10px] px-1.5 py-0.5 rounded ${slide.active ? "bg-emerald-500/80 text-gray-900" : "bg-gray-500/80 text-gray-900"}`}>
