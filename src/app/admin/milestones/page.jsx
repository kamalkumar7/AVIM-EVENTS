    ...
    <div className="p-8 w-full">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Global achievements shown on the About page.</p>
        </div>
      ...
      <div className="space-y-3">
        {items.map((item) => (
          <div key={item.id} className={`flex gap-4 items-start bg-white border ${item.featured ? "border-amber-500/40" : "border-gray-200"} rounded-xl p-5 ${!item.active ? "opacity-50" : ""}`}>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? "Edit Milestone" : "Add Milestone"}>
        <div className="space-y-4">
          <div>
            ...
            <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm" placeholder="e.g. Dubai Work – Palazzo Versace" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-gray-500 mb-1">Badge</label>
              <input value={form.badge} onChange={(e) => setForm({ ...form, badge: e.target.value })} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm" placeholder="e.g. Dubai" />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Location</label>
              <input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm" placeholder="e.g. Dubai" />
            </div>
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1">Description</label>
            <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={3} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm resize-none" />
          </div>
          <div className="flex gap-4">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm text-gray-600">Featured (large card)</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
