    ...
    <div className="p-8 w-full">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Leadership & execution team shown on the About page.</p>
        </div>
        ...
        {items.map((item, i) => (
          <div key={item.id} className={`flex items-center gap-4 bg-white border border-gray-200 rounded-xl px-5 py-4 ${!item.active ? "opacity-50" : ""}`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-gray-500 text-xs">{item.role}{item.location ? ` • ${item.location}` : ""}</p>
            </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm" />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Location</label>
              <input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm" placeholder="e.g. Bangalore" />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Initials</label>
              <input value={form.initials} onChange={(e) => setForm({ ...form, initials: e.target.value })} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-900 text-sm" placeholder="e.g. SA" maxLength={3} />
            </div>
          </div>
          <div>
