    ...
    <div className="space-y-2">
      ...
      {preview && (
        <div className="flex gap-2 items-center">
          <input value={preview} onChange={(e) => { setPreview(e.target.value); onChange(e.target.value); }} className="flex-1 bg-gray-50 border border-gray-200 rounded px-2 py-1 text-xs text-gray-700" placeholder="or paste URL" />
          <button type="button" onClick={() => { setPreview(""); onChange(""); }} className="text-xs text-red-500 hover:text-red-600">Remove</button>
        </div>
      )}
    </div>
  );
